...
type Track = {
  id: string; source: "spotify" | "musicbrainz"; name: string; artist: string; album: string; releaseYear: string; durationLabel: string; popularity: number; imageUrl: string | null; spotifyUrl: string; availableMarkets: string[];
};
    ...
    <main className="min-h-screen overflow-hidden bg-[#060609] text-white selection:bg-[#ff2db2] selection:text-black">
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-7 sm:px-8 lg:px-12">
        ...
        <section className="grid gap-10 py-14 lg:grid-cols-[1fr_320px] lg:items-end">
          ...
          <div className="hud-panel p-4"><p className="mb-3 font-mono text-[10px] uppercase tracking-[.25em] text-white/40">Backend status</p><div className="flex items-center gap-2 font-mono text-xs"><span className={`h-2 w-2 rounded-full ${status.data?.spotifyConfigured ? "bg-[#55ff9a] shadow-[0_0_10px_#55ff9a]" : "bg-[#ffb000] shadow-[0_0_10px_#ffb000]"}`} />{status.data?.spotifyConfigured ? "SPOTIFY LINK // ONLINE" : "MUSICBRAINZ LINK // ONLINE"}</div></div>
        </section>
        ...
        {!status.data?.spotifyConfigured && <div className="hud-panel mb-10 border-[#ffb000]/40 bg-[#ffb000]/5 p-5 font-mono text-xs leading-6 text-[#ffd37a]"><span className="mr-2 text-[#ffb000]">[NOTICE]</span> Spotify está sin credenciales por ahora. El buscador usa MusicBrainz y Cover Art Archive sin claves; cuando agregues SPOTIFY_CLIENT_ID y SPOTIFY_CLIENT_SECRET, pasará automáticamente a Spotify.</div>}
        ...
        {search.error && <div className="hud-panel mb-8 border-[#ff4d6d]/50 bg-[#ff4d6d]/5 p-5 font-mono text-xs leading-6 text-[#ff9aaa]"><span className="mr-2 text-[#ff4d6d]">[ERROR]</span> No se pudo consultar la fuente musical activa. Reintentá más tarde o verificá la configuración del backend.</div>}
        {tracks.length > 0 ? <><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{tracks.map(track => <button key={track.id} onClick={() => setSelected(track)} className="hud-card group text-left"><div className="aspect-square overflow-hidden bg-[#11121a]">{track.imageUrl ? <img src={track.imageUrl} alt={`Carátula de ${track.album}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-105 group-hover:opacity-80" /> : <div className="grid h-full place-items-center text-[#ff2db2]"><Headphones /></div>}</div><div className="p-4"><div className="mb-3 flex items-center justify-between gap-3"><span className="border border-[#00e7ff]/30 px-2 py-1 font-mono text-[9px] uppercase tracking-wider text-[#00e7ff]">{track.source === "spotify" ? "Spotify" : "MusicBrainz"}</span><span className="font-mono text-[9px] text-white/35">{track.popularity ? `${track.popularity}/100` : "CATALOG"}</span></div><p className="truncate font-display font-bold">{track.name}</p><p className="mt-1 truncate text-sm text-[#00e7ff]">{track.artist}</p><div className="mt-4 flex justify-between font-mono text-[10px] uppercase tracking-wider text-white/40"><span>{track.releaseYear}</span><span>{track.durationLabel}</span></div></div></button>)}</div>{search.data?.nextOffset !== null && search.data?.nextOffset !== undefined && <div className="mt-10 text-center"><Button onClick={loadMore} disabled={search.isFetching} variant="outline" className="rounded-none border-[#00e7ff]/50 bg-transparent font-mono text-xs uppercase tracking-[.2em] text-[#00e7ff] hover:bg-[#00e7ff]/10">{search.isFetching ? "Scanning..." : "Ver más resultados"}</Button></div>}</> : submitted && !search.isFetching && <div className="hud-panel py-20 text-center font-mono text-sm text-white/45">No se encontraron señales para esta búsqueda en {status.data?.spotifyConfigured ? "Spotify" : "MusicBrainz"}.</div>}
