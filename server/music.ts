...
async function searchJamendo(query: string, limit: number) { const clientId = env("JAMENDO_CLIENT_ID"); if (!clientId) return null; const response = await axios.get("https://api.jamendo.com/v3.0/tracks/", { params: { client_id: clientId, format: "json", search: query, limit, offset: 0, audioformat: "mp31" }, timeout: 10_000 }); const items = (response.data?.results ?? []).filter((track: any) => track?.id && track?.name && track?.audio).map(toJamendoTrack); return { configured: true, source: "jamendo" as const, items, total: response.data?.headers?.results_count ?? items.length, nextOffset: null }; }
