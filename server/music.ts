...
async function searchJamendo(query: string, limit: number) { const clientId = env("JAMENDO_CLIENT_ID"); if (!clientId) return null; const response = await axios.get("https://api.jamendo.com/v3.0/tracks/", { params: { client_id: clientId, format: "json", search: query, limit, offset: 0, audioformat: "mp31" }, timeout: 5_000 }); const items = (response.data?.results ?? []).filter((track: any) => track?.id && track?.name && track?.audio).map(toJamendoTrack); return { configured: true, source: "jamendo" as const, items, total: response.data?.headers?.results_count ?? items.length, nextOffset: null }; }

async function safeJamendoSearch(query: string, limit: number) { try { return await searchJamendo(query, limit); } catch (error) { console.warn("[Jamendo] Search unavailable:", error instanceof Error ? error.message : "unknown error"); return null; } }
...
export function emptySearchResult(source: "spotify" | "musicbrainz" | "audius" | "jamendo" = "musicbrainz") { return { configured: true, source, items: [] as MusicTrack[], total: 0, nextOffset: null }; }
...
export async function searchMusic(query: string, offset: number, limit = 12) { const intent = interpretQuery(query); try { if (intent.type === "free" && offset === 0) { const fastQuery = (intent.terms ?? query).split(/\s+/).filter(word => word.length > 2).slice(0, 2).join(" "); const fastJamendo = await safeJamendoSearch(fastQuery, limit); const fastItems = fastJamendo ? fastJamendo.items.filter((item: MusicTrack) => matchesJamendoQuery(item, intent, intent.terms ?? query)).map((item: MusicTrack) => ({ ...item, matchMode: "discovery" as const })) : []; if (fastItems.length) return { ...fastJamendo!, items: rankMusicTracks(fastItems, intent.terms ?? query), total: fastItems.length }; } const primary = intent.type === "artist" || intent.type === "album" || intent.type === "track" ? await searchMusicBrainz(query, offset, limit) : (await safeSpotifySearch(query, offset, limit)) ?? await searchMusicBrainz(query, offset, limit); if (primary.items.some((item: MusicTrack) => Boolean(item.previewUrl)) || offset > 0) return primary; const fallbackQuery = intent.artist ?? intent.terms ?? query; const audius = await safeAudiusSearch(fallbackQuery, limit); const matchedAudius = audius ? audius.items.filter((item: MusicTrack) => matchesAudioQuery(item, fallbackQuery) && primary.items.some((candidate: MusicTrack) => matchesAudioCandidate(candidate, item, fallbackQuery))) : []; if (matchedAudius.length) return { ...audius!, items: rankMusicTracks(matchedAudius, fallbackQuery), total: matchedAudius.length }; const jamendo = await safeJamendoSearch(fallbackQuery, limit); const matchedJamendo = jamendo ? jamendo.items.filter((item: MusicTrack) => matchesJamendoCandidate(primary.items, item, intent, fallbackQuery)) : []; const jamendoItems = matchedJamendo.map((item: MusicTrack) => ({ ...item, matchMode: primary.items.some((candidate: MusicTrack) => matchesAudioCandidate(candidate, item, fallbackQuery)) ? "strict" as const : "discovery" as const })); return jamendoItems.length ? { ...jamendo!, items: rankMusicTracks(jamendoItems, fallbackQuery), total: jamendoItems.length } : primary; } catch (error) { console.warn("[Music] Primary source unavailable, using fallback:", error instanceof Error ? error.message : "unknown error"); const fallbackQuery = intent.artist ?? intent.terms ?? query; const audius = await safeAudiusSearch(fallbackQuery, limit); const matchedAudius = audius ? audius.items.filter((item: MusicTrack) => matchesAudioQuery(item, fallbackQuery)) : []; if (matchedAudius.length) return { ...audius!, items: rankMusicTracks(matchedAudius, fallbackQuery), total: matchedAudius.length }; const jamendo = await safeJamendoSearch(fallbackQuery, limit); const matchedJamendo = jamendo ? jamendo.items.filter((item: MusicTrack) => matchesJamendoQuery(item, intent, fallbackQuery)).map((item: MusicTrack) => ({ ...item, matchMode: "discovery" as const })) : []; return matchedJamendo.length ? { ...jamendo!, items: rankMusicTracks(matchedJamendo, fallbackQuery), total: matchedJamendo.length } : emptySearchResult(); } }
