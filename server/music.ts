...
export async function searchSpotify(query: string, offset: number, limit = 12) {
  const token = await getSpotifyToken();
  if (!token) return { configured: false, items: [] as MusicTrack[], total: 0, nextOffset: null as number | null };
  const response = await axios.get("https://api.spotify.com/v1/search", {
    params: { q: query, type: "track", limit, offset, market: "US" },
    headers: { Authorization: `Bearer ${token}` },
    timeout: 10_000,
  });
  const tracks = response.data.tracks;
  const items: MusicTrack[] = tracks.items.map((track: any) => ({
    id: track.id,
    name: track.name,
    ...
    durationLabel: durationLabel(track.duration_ms),
  ...
  return { configured: true, items, total: tracks.total, nextOffset: offset + items.length < tracks.total ? offset + items.length : null };
}
...
export async function getGeniusLyrics(name: string, artist: string): Promise<LyricsResult> {
  ...
  try {
    ...
    return {
      ...
      sourceUrl: hit.url,
      sourceName: "Genius",
      message: "Genius autoriza la búsqueda y los metadatos, pero su API pública no entrega el texto completo de las letras. Abrí la página original para leerlas allí.",
    };
  } catch {
    return { status: "unavailable", text: null, sourceUrl: null, sourceName: null, message: "Genius no respondió. Podés reintentar en unos segundos." };
  }
}
