...
import { getAudioProviderHealth, getLrclibLyrics, isSpotifyConfigured, searchMusic, searchOpenCatalog } from "./music.js";
...
export const appRouter = router({
  ...
  music: router({
    ...
    openCatalog: publicProcedure
      .input(z.object({ offset: z.number().int().min(0).max(1000).default(0), limit: z.number().int().min(1).max(40).default(24) }))
      .query(async ({ input }) => ({ query: "music", ...(await searchOpenCatalog(input.offset, input.limit)) })),
